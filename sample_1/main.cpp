...
int main()
{
	...
	uint64_t mhartid = _CSRR(mhartid);
	lprintf("MHARDID:%08X%08X\n\r", uint32_t(mhartid >> 32), uint32_t(mhartid));
	uint64_t misa = _CSRR(misa);
	lprintf("MISA:%08X%08X\n\r", uint32_t(misa >> 32), uint32_t(misa));
	...
	for (;;) {
		...
		uint32_t pvalue = *(volatile uint32_t*)(0x10013000 + 0x14);
		lprintf("IP:%08X\n\r", pvalue);
...
{
	s_intr_count++;
	IoPort::write64(MTIMECMP, s_intr_count * 0x40000ull);
